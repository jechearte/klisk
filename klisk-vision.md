# Klisk — Vision & Architecture

> **Un framework para construir agentes de IA de forma programática.**
> Como Remotion es para vídeos, Klisk es para agentes.

---

## Qué es Klisk

Klisk es una **capa de abstracción sobre el OpenAI Agents SDK** que ofrece:
...
Klisk **no contiene inteligencia artificial propia**. Es infraestructura pura. El usuario utiliza su propio agente de IA (Claude, ChatGPT, etc.) para generar código que sigue las convenciones de Klisk, y usa el Studio para visualizar y probar el resultado.
...
| Remotion no tiene ningún agente de IA | Klisk no tiene ningún agente de IA |
| El usuario usa Claude para que genere componentes React | El usuario usa Claude para que genere código Klisk |
| Remotion expone primitivas: `<Composition>`, `<Sequence>`, `useCurrentFrame()` | Klisk expone primitivas: `define_agent()`, `@tool`, handoffs, guardrails |
| `npx remotion preview` abre Remotion Studio en localhost | `klisk dev` abre Klisk Studio en localhost |
| `npx remotion render` renderiza el vídeo | `klisk run` ejecuta el agente |
| El valor de Remotion es el framework, no la IA | El valor de Klisk es el framework, no la IA |
...
```
1. $ klisk create travel-bot && cd travel-bot

2. $ klisk dev
   → Se abre Studio en localhost:3000

3. El usuario le pide a su agente de IA (Claude, etc.):
   "Estoy en un proyecto Klisk. Crea un agente que busque
    vuelos, compare precios y ayude a reservar."

4. El agente de IA genera los archivos siguiendo las convenciones de Klisk
...
7. Cuando está satisfecho:
   $ klisk export --format standalone ./output
   → Código Python limpio con openai-agents, sin dependencia de Klisk
```
...
Una forma estándar y opinada de estructurar un proyecto de agentes. Igual que Remotion dice "un vídeo es un componente React que recibe props y usa `useCurrentFrame()`", Klisk dice **"un agente es un módulo Python que exporta una configuración estándar"**.
...
Una interfaz web que se levanta con `klisk dev` y permite **visualizar y probar** el agente que estás construyendo. **No genera código** — solo refleja lo que hay en disco.

```
┌─────────────────────────────────────────────────────┐
│  Klisk Studio              localhost:3000         │
├──────────────────────┬──────────────────────────────┤
│                      │                              │
...
klisk run "Busca vuelos Madrid-NYC en marzo"
...
klisk export --format api ./output

# Exportar como paquete Python puro (sin Klisk)
klisk export --format standalone ./output
# → Genera código directo con openai-agents, sin dependencia de Klisk
```
...
4. **`klisk check`** — el LLM puede ejecutar esto para validar que lo que generó es correcto

---

## Primitivas del OpenAI Agents SDK (por debajo)

Klisk es un wrapper delgado sobre el OpenAI Agents SDK. Estas son las primitivas del SDK que Klisk expone de forma simplificada:

| Primitiva SDK | Wrapper Klisk | Descripción |
|---|---|---|
...
| `Runner.run()` / `Runner.run_streamed()` | `klisk run` / Studio chat | Ejecuta el bucle del agente (LLM → tools → handoffs → respuesta) |
...
Klisk **no construye IA**. Construye **la infraestructura para que la IA del usuario pueda construir agentes de forma predecible**.
