...
def _check_cloud_run_api(gcp_project: str) -> None:
    """Check if Cloud Run API is enabled, and offer to enable it."""
    try:
        result = _run_gcloud([
            "services", "list",
            "--project", gcp_project,
        ...
        if "run.googleapis.com" not in result.stdout:
            typer.echo("  Cloud Run API is not enabled for this project.\n")
            enable = typer.confirm("  Enable it now?", default=True)
            if enable:
                typer.echo("  Enabling Cloud Run API (this may take a moment)...")
                ...
                if enable_result.returncode != 0:
                    typer.echo(f"\nError: Could not enable Cloud Run API.\n", err=True)
                    typer.echo("  Enable it manually at:", err=True)
                    typer.echo(f"    https://console.cloud.google.com/apis/library/run.googleapis.com?project={gcp_project}", err=True)
                    raise typer.Exit(1)
                typer.echo("  Cloud Run API enabled.\n")
            else:
                typer.echo("\nError: Cloud Run API is required.\n", err=True)
                typer.echo("  Enable it manually:", err=True)
                typer.echo("    gcloud services enable run.googleapis.com", err=True)
                raise typer.Exit(1)
    except subprocess.TimeoutExpired:
        pass  # Non-critical
    except typer.Exit:
...
def deploy_init(
    ...
    if not config_file.exists():
        ...
        typer.echo("  Make sure you're in an AgentKit project directory, or specify the project name:", err=True)
        typer.echo("    agentkit deploy init <project-name>", err=True)
        raise typer.Exit(1)
    ...
    typer.echo("    2. Run: agentkit deploy")
...
def deploy(
    ...
    if not dockerfile_path.exists():
        ...
        typer.echo("    agentkit deploy init", err=True)
        raise typer.Exit(1)
    ...
    _check_cloud_run_api(gcp_project)
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            typer.echo("  Common causes:", err=True)
            typer.echo("    - Billing not enabled for the project", err=True)
            typer.echo("    - Insufficient permissions (you need Cloud Run Admin role)", err=True)
            typer.echo("    - Artifact Registry API not enabled", err=True)
            typer.echo("", err=True)
            typer.echo("  Try enabling the required APIs:", err=True)
            typer.echo("    gcloud services enable run.googleapis.com artifactregistry.googleapis.com cloudbuild.googleapis.com", err=True)
            raise typer.Exit(1)
    except subprocess.TimeoutExpired:
