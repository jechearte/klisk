...
```bash
klisk serve [name_or_path] --port 8080 --host 0.0.0.0
```
...
KLISK_WIDGET_KEY=widget-key
```

| Variable | Description |
|---|---|
| `KLISK_API_KEY` | General-purpose key(s) |
| `KLISK_CHAT_KEY` | Chat UI key(s) |
| `KLISK_WIDGET_KEY` | Widget key(s) |
...
**CLI:** Use `@path` syntax: `klisk run -p my-agent "@photo.jpg Describe this"`
